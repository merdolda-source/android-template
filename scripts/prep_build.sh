...
set -e
##########################################################################
# ULTRA APP V200 - THE COLOSSUS (FULL SOURCE CODE - NO COMPRESSION)
# 
# BU SCRIPT ŞUNLARI İÇERİR:
# 1. ImageMagick ile İkon İşleme
# 2. Gradle ve SDK Sürüm Düzeltmeleri
# 3. AndroidManifest (TV Leanback + Touchscreen Bypass)
# 4. AdsManager (Unity Ads Tam Entegrasyon)
# 5. MainActivity (Dinamik Menü, Grid/List/Card Yapısı, JSON Fetch)
# 6. ChannelListActivity (M3U Parser, JSON Flat List, Header Parsing, UI Styling)
# 7. PlayerActivity (ExoPlayer, Deep Link Resolve, Headers, Fullscreen, Rotate)
# 8. WebViewActivity (Basic Browser)
##########################################################################
...
echo "=================================================="
echo "   ULTRA APP V200 - THE COLOSSUS DEPLOYING..."
echo "=================================================="

# --------------------------------------------------------
# 0. SİSTEM HAZIRLIĞI VE ARAÇLAR
# --------------------------------------------------------
echo "⚙️ [1/10] Sistem araçları güncelleniyor..."
...
# 1. TEMİZLİK
# --------------------------------------------------------
echo "🧹 [2/10] Eski dosyalar temizleniyor..."
...
# --------------------------------------------------------
# 2. İKON İŞLEME (SAĞLAM CONVERT)
# --------------------------------------------------------
echo "🖼️ [3/10] İkon işleniyor..."
ICON_TARGET="app/src/main/res/mipmap-xxxhdpi/ic_launcher.png"
TEMP_FILE="downloaded_icon_raw"

# Tarayıcı gibi davranarak indir (403 hatasını önler)
curl -s -L -k -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36" -o "$TEMP_FILE" "$ICON_URL" || true

if [ -s "$TEMP_FILE" ] && [ $(stat -c%s "$TEMP_FILE") -gt 500 ]; then
    # ImageMagick ile resize ve convert işlemi
    convert "$TEMP_FILE" -resize 512x512! -background none -flatten "$ICON_TARGET" || cp "$TEMP_FILE" "$ICON_TARGET"
else
    # İndirilemezse varsayılan bir Android logosu koy
...
# Son güvenlik: Dosya hala yoksa mavi bir kare oluştur
if [ ! -s "$ICON_TARGET" ]; then 
    convert -size 512x512 xc:blue "$ICON_TARGET"; 
fi

# --------------------------------------------------------
# 3. SETTINGS.GRADLE (REPO ÇAKIŞMA ÇÖZÜMÜ)
# --------------------------------------------------------
echo "📦 [4/10] Gradle ayarları yapılıyor..."
...
# --------------------------------------------------------
# 5. ANDROID MANIFEST (TV + TOUCHSCREEN FIX + FULLSCREEN)
# --------------------------------------------------------
echo "📜 [5/10] Manifest dosyası oluşturuluyor..."
...
# --------------------------------------------------------
# 6. ADS MANAGER (REKLAM YÖNETİMİ)
# --------------------------------------------------------
echo "💰 [6/10] Reklam modülü yazılıyor..."
...
public class AdsManager {
    ...
    private static boolean EN = false, B = false;
    private static String GID = "", BID = "", IID = "";

    public static void init(Activity a, JSONObject j) {
        try {
            if (j == null) return;
            EN = j.optBoolean("enabled", false);
            GID = j.optString("game_id");
            B = j.optBoolean("banner_active");
            BID = j.optString("banner_id");
            IID = j.optString("inter_id");
            F = j.optInt("inter_freq", 3);
            
            if (EN && !GID.isEmpty()) {
                UnityAds.initialize(a.getApplicationContext(), GID, false, null);
            }
        } catch (Exception e) {}
    }
...
# --------------------------------------------------------
# 7. MAIN ACTIVITY (ANA MENÜ)
# --------------------------------------------------------
echo "📱 [7/10] Main Activity yazılıyor..."
...
public class MainActivity extends Activity {
    ...
    // Panelden gelen yeni liste tasarım değişkenleri
    private String listType="CLASSIC", listItemBg="#FFFFFF", listIconShape="SQUARE"; 
    private int listRadius=0;
...
# --------------------------------------------------------
# 8. CHANNEL LIST ACTIVITY (PARSER + DESIGN + CLICK FIX)
# --------------------------------------------------------
echo "📋 [8/10] Kanal Listesi mantığı işleniyor..."
...
public class ChannelListActivity extends Activity {
    ...
    private String hC, bC, tC, pCfg, fC, lType, lBg, lIcon; 
    private int lRad;
    private TextView title;

    class Item { 
        String n,u,i,h; 
        Item(String name,String url,String img,String head){n=name;u=url;i=img;h=head;} 
    }
    ...
    class Load extends AsyncTask<String,Void,String> {
        ...
        protected void onPostExecute(String r) {
            ...
            try {
                ...
                // JSON: GRUP YOK SAYILIR (FLAT LISTE)
                if("JSON_LIST".equals(t) || r.trim().startsWith("{")) {
                    ...
                    for(int i=0;i<arr.length();i++){
                        ...
                        // JSON'da da referer varsa al
                        String ref = o.optString("referer", "");
                        if(!ref.isEmpty()) head.put("Referer", ref);
...
# --------------------------------------------------------
# 9. PLAYER ACTIVITY (FULL SCREEN + DEEP LINK + HEADER)
# --------------------------------------------------------
echo "🎥 [9/10] Player motoru yazılıyor..."
...
public class PlayerActivity extends Activity {
    ...
    protected void onCreate(Bundle s) {
        ...
        // 1. TAM EKRAN VE SİYAH BARLARI KALDIRMA
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
        getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        
        // Immersive Mode (Sanal tuşları gizler)
        ...
        // 2. PLAYER AYARLARI (RESIZE / ROTATE / WATERMARK)
        try {
            JSONObject cfg = new JSONObject(getIntent().getStringExtra("PLAYER_CONFIG"));
            
            // Resize Mode (Siyah barları yok etme)
            ...
            // Auto Rotate (Sensör)
            if(!cfg.optBoolean("auto_rotate", true)) setRequestedOrientation(0); // Zorunlu Yatay
    ...
    // 3. DEEP LINK RESOLVER (UZANTISIZ LİNKLERİ ÇÖZER)
    ...
    // 4. PLAYER BAŞLATMA (EXOPLAYER + HEADERS)
...
echo "✅ ULTRA APP V200 - DEPLOYMENT SUCCESSFUL!"
