...
if [ -s "$TEMP_FILE" ] && [ $(stat -c%s "$TEMP_FILE") -gt 500 ]; then
    echo "✅ İkon indirildi, formatlanıyor..."
    convert "$TEMP_FILE" -resize 512x512! -background none -flatten "$ICON_TARGET" || cp "$TEMP_FILE" "$ICON_TARGET"
else
    echo "⚠️ İkon bulunamadı, varsayılan oluşturuluyor..."
    convert -size 512x512 xc:#2196F3 -fill white -gravity center -pointsize 100 -annotate 0 "TV" "$ICON_TARGET"
fi
    ...
    public static void init(Activity activity, JSONObject config) {
        try {
            ...
            if (isEnabled && !gameId.isEmpty()) {
                UnityAds.initialize(activity.getApplicationContext(), gameId, isTestMode, new IUnityAdsInitializationListener() {
                    @Override
                    public void onInitializationComplete() { Log.d("AdsManager", "Unity Ads Initialized"); }
                    @Override
                    public void onInitializationFailed(UnityAds.UnityAdsInitializationError error, String message) { Log.e("AdsManager", "Init Failed: " + message); }
                });
            }
        } catch (Exception e) {
            e.printStackTrace();
...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        root.addView(splash, new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        ...
        headerLayout.addView(titleTxt, new LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1.0f));
        ...
        // Add Header to Root
        RelativeLayout.LayoutParams hp = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        hp.addRule(RelativeLayout.ALIGN_PARENT_TOP); 
        root.addView(headerLayout, hp);

        // 3. Scroll View & Container
        ...
        RelativeLayout.LayoutParams sp = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    ...
    private void addBtn(String txt, String type, String url, String cont, String ua, String ref, String org) {
        ...
        if(menuType.equals("GRID")) {
            ...
            LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, 250, 1.0f); 
...
# --------------------------------------------------------
# 8. CHANNEL LIST ACTIVITY (DESIGN ENGINE + GAP FIX)
# --------------------------------------------------------
echo "📋 [8/10] ChannelList (Tasarım Motoru) yazılıyor..."
...
public class ChannelListActivity extends Activity {
    ...
    protected void onCreate(Bundle s) {
        ...
        // !CRITICAL FIX: Weight=1 fills the screen properly!
    ...
    class Load extends AsyncTask<String,Void,String> {
        ...
        protected void onPostExecute(String r) {
            ...
            try { 
                ...
                // JSON PARSING (Flat List Strategy)
...
# --------------------------------------------------------
# 9. PLAYER ACTIVITY (DEEP CORE)
# --------------------------------------------------------
echo "🎥 [9/10] Player Core (ExoPlayer + Deep Resolver) yazılıyor..."
...
public class PlayerActivity extends Activity {
    ...
    protected void onCreate(Bundle s) {
        ...
        try { 
            ...
            // Orientation
            if(!cfg.optBoolean("auto_rotate", true)) setRequestedOrientation(0); // Force Landscape
    ...
    private class ResolveUrlTask extends AsyncTask<String, Void, UrlInfo> {
        protected UrlInfo doInBackground(String... params) {
            ...
            try { 
                ...
                // Redirect Follower (Deep Resolver)
                for (int i = 0; i < 5; i++) {
